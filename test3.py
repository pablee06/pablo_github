...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.original_image = pygame.image.load('tank.png')
...
def enter_player_name():
    ...
    while entering_name:
        ...
        screen.fill(BLACK)
        enter_name_text = font.render("Name eingeben:", True, BLUE)
...
while running:
    ...
    if not show_ranking and not game_over:
        ...
        time_text = font.render("Spielzeit: {:02d}:{:02d}".format(minutes, seconds), True, BLUE)
        screen.blit(time_text, (10, 10))

    if not show_ranking and not game_over:
        coins_text = font.render("Gesammelte Münzen: {}".format(player.collected_coins), True, BLUE)
        screen.blit(coins_text, (10, 50))

    extra_lives_text = font.render("Extra-Leben: {}".format(player.extra_lives), True, BLUE)
    ...
    if game_over:
        ...
        if len(best_times) > 0:
            ...
            for i, (name, time) in enumerate(best_times):
                rank = font.render("{}. {} - {:.2f} Sekunden".format(i + 1, name, time), True, BLUE)
